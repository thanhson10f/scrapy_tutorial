...
class QuotesSpider(scrapy.Spider):
    ...
    def parse_category(self, response):
        ...
        for link in question_links:
            ...
            # print(number + " - " + url)
            yield scrapy.Request(url=url, callback=self.parse, meta={'number': number}, dont_filter=True)
    ...
    def parse(self, response):
        ...
        enids = content.css('div.right-options ul#options-parent li').xpath('@enid').extract()
        for enid in enids:
            url = 'http://gmat.kmf.com/question/{0}.html'.format(enid)
            yield scrapy.Request(url=url, callback=self.parse_sub, meta={'enid': enid})
    ...
    def parse_sub(self, response):
        ...
        choice_question = ''.join(content.css('div#parent-cont div.tab-cont>p').xpath('node()').extract()).strip()
